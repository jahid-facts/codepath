...
import { dockerModules, dockerTopics, dockerLabs } from '../app/courses/docker.js'

test('Docker curriculum has eight modules, each with at least one topic', () => {
  assert.equal(dockerModules.length, 8)
  assert.equal(new Set(dockerModules.map((module) => module.id)).size, 8)
  for (const module of dockerModules) {
    assert.ok(module.title.en.trim() && module.title.bn.trim(), `${module.id} needs a bilingual title`)
    assert.ok(dockerTopics.some((topic) => topic.moduleId === module.id), `${module.id} has no topics`)
  }
})

test('Docker topics are unique, ordered, and every topic maps to a real module', () => {
  const moduleIds = new Set(dockerModules.map((module) => module.id))
  assert.ok(dockerTopics.length >= 36, 'the deep Docker course should have at least 36 topics')
  assert.equal(new Set(dockerTopics.map((topic) => topic.id)).size, dockerTopics.length)
  dockerTopics.forEach((topic, index) => {
...
test('every Docker topic has bilingual content, objectives, and fifteen valid questions', () => {
  for (const topic of dockerTopics) {
...
test('Docker command tables are bilingual and carry a command', () => {
  for (const topic of dockerTopics) {
...
test('Docker hands-on examples are bilingual with real code', () => {
  const withExample = dockerTopics.filter((topic) => topic.example)
  assert.ok(withExample.length >= 12, 'the deep course should include many hands-on code examples')
...
test('Docker single-answer exams distribute the correct option across all positions', () => {
  const positions = new Set(dockerTopics.flatMap((topic) => topic.exam.filter((question) => question.type === 'single').map((question) => question.correct[0])))
  assert.deepEqual([...positions].sort(), ['a', 'b', 'c', 'd'])
})

test('Docker guided labs are bilingual with a valid expert choice per stage', () => {
  assert.ok(dockerLabs.length >= 6, 'the deep course should include at least six labs')
