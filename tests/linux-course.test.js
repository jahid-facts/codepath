...
import { linuxModules, linuxTopics, linuxLabs } from '../app/courses/linux.js'

test('Linux curriculum has seven modules, each with at least one topic', () => {
  assert.equal(linuxModules.length, 7)
  assert.equal(new Set(linuxModules.map((module) => module.id)).size, 7)
  for (const module of linuxModules) {
    assert.ok(module.title.en.trim() && module.title.bn.trim(), `${module.id} needs a bilingual title`)
    assert.ok(linuxTopics.some((topic) => topic.moduleId === module.id), `${module.id} has no topics`)
  }
})

test('Linux topics are unique, ordered, and every topic maps to a real module', () => {
  const moduleIds = new Set(linuxModules.map((module) => module.id))
  assert.equal(new Set(linuxTopics.map((topic) => topic.id)).size, linuxTopics.length)
  linuxTopics.forEach((topic, index) => {
...
test('every Linux topic has bilingual content, objectives, and fifteen valid questions', () => {
...
test('Linux command tables are bilingual and carry a command', () => {
  for (const topic of linuxTopics) {
...
test('Linux single-answer exams distribute the correct option across all positions', () => {
  const positions = new Set(linuxTopics.flatMap((topic) => topic.exam.filter((question) => question.type === 'single').map((question) => question.correct[0])))
  assert.deepEqual([...positions].sort(), ['a', 'b', 'c', 'd'])
})

test('Linux guided labs are bilingual with a valid expert choice per stage', () => {
  assert.ok(linuxLabs.length >= 4)
