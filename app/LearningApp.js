  ...
  return <article className="lesson-page">
    ...
    <div className="lesson-layout">
      <div className="lesson-content">
        ...
        {topic.complexity?.length > 0 && <LessonSection number="•" label={course.id === 'git' ? (lang === 'bn' ? 'জরুরি কমান্ড' : 'Key commands') : course.id === 'networking' ? (lang === 'bn' ? 'দ্রুত রেফারেন্স' : 'Quick reference') : (lang === 'bn' ? 'জটিলতা' : 'Complexity')}><ComplexityTable rows={topic.complexity} lang={lang} courseId={course.id} /></LessonSection>}
        ...
        <div className="exam-cta"><div><span className="eyebrow">{lang === 'bn' ? 'জ্ঞান যাচাই' : 'Knowledge check'}</span><h2>{lang === 'bn' ? `${topic.exam.length} প্রশ্নের পরীক্ষার জন্য প্রস্তুত?` : `Ready for the ${topic.exam.length}-question exam?`}</h2><p>{lang === 'bn' ? 'সাবমিট করার পর প্রতিটি উত্তরের ব্যাখ্যা দেখুন।' : 'Submit all answers, then review an explanation for each one.'}</p></div><button className="primary-button" onClick={() => navigate('exam', { topicId: topic.id })}>{lang === 'bn' ? 'পরীক্ষা শুরু করুন' : 'Start exam'} <Icon name="arrow" /></button></div>
      </div>
...
function Exam({ topicId, progress, setProgress, lang, navigate, openTopic }) {
  ...
  return <div className="exam-page">
    ...
    {!submitted ? <>
      <PageIntro eyebrow={`${lang === 'bn' ? 'টপিক পরীক্ষা' : 'Topic exam'} · ${String(topic.order).padStart(2, '0')}`} title={text(topic.title)} description={lang === 'bn' ? `সব ${topic.exam.length}টি প্রশ্নের উত্তর দিন। সাবমিটের পর উত্তর ও ব্যাখ্যা প্রকাশ পাবে।` : `Answer all ${topic.exam.length} questions. Answers and explanations appear only after submission.`} action={<div className="exam-progress"><strong>{answered}/{topic.exam.length}</strong><small>{lang === 'bn' ? 'উত্তর দেওয়া' : 'answered'}</small></div>} />
